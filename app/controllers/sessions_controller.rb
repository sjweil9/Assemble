    ...
    def create
        ...
        unless user.authenticate(params[:password])
            ...
            if user.login_attempts < 5
                ...
                flash[:password] << "You have now made #{user.login_attempts} failed attempts. You have #{5 - user.login_attempts} more attempts before being locked out."
            else
                user.update(account_locked: true, unlock_token: SecureRandom.urlsafe_base64.to_s)
                flash[:password] = ["You have now failed 5 login attempts within the last hour and your account has been locked. An email will be sent to #{user.email} with instructions to unlock your account."]
